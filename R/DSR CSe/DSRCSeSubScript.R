...
# Plot full scatterplot matrix
fileName <- paste(ttl, "_Scatter_Matrix-Initial.pdf", sep="")
...
fileName <- paste(ttl, "_Scatter_Matrix-Final.pdf", sep="")
pdf(file=fileName, width=6, height=6, family="Times")
...
par(mar=c(5.1,4.1,1.1,1.1))
hist(resid, freq=FALSE, main="", xlab=expression(Residual~C[Se]~(mu*g~L^{-1})), col="grey90", xlim=xlimit, ylim=ylimit)#, add=TRUE)
